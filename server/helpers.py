...
def create_user(username, clerk_id):
    result, count = app.supabase.table('users').insert({"username": username, "clerk_id": clerk_id, "groups": []}).execute()
    return {"message": "User created", "id": result[1][0]['id'], "username": result[1][0]['username']}

def get_user(clerk_id):
    data, count = app.supabase.table('users').select("id, username, groups").eq('clerk_id', clerk_id).execute()
    response = { "id": data[1][0]["id"], "username": data[1][0]["username"], "groups": []}
    for i in data[1][0]["groups"]:
        group, _count = app.supabase.table('groups').select("group_name", "description").eq('id', i).execute()
...
def create_group(group_name, group_description, user_id, username, current_groups):
    result, count = app.supabase.table('groups').insert({
...
def add_user_to_group(user_id, group_id, username, current_groups):
    group, _count = app.supabase.table('groups').select('members').eq('id', group_id).execute()
    ...
    group_members, _count = app.supabase.table('groups').update({
       "members": updated_members
         }).eq('id', group_id).execute() 
       
    result, count = app.supabase.table('groupMembers').insert({
        "user_id": user_id,
        "group_id": group_id,
        "current_balance": 0.0,
        "username": username
    ...
    result, count = app.supabase.table('users').update({
...
def get_group_data(group_id):
    data, count = app.supabase.table('groups').select(
        "group_name",
        "description",
        "members"
        ).eq('id', group_id).execute()
...
def get_group_expenses(group_id):
    expenses, count = app.supabase.table('expenses').select("*").eq('group_id', group_id).execute()
    return expenses[1]

def get_group_members(members, group_id):
    member_data = []
    for member in members:
        group_member, _count = app.supabase.table('groupMembers').select('*').eq(
